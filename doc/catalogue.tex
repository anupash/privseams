...
necessary in the kernel based approach. See
\autoref{sec:daemon_architecture} for a more detailed description of the
chained code.
\\&
...
daemon, were ported into kernelspace as a separate crypto module. This
reduces the need for a HIP daemon to a minimum and should accelerate
the performance of the HIP kernel module a bit. The most important
benifit is that the kernel module code could be simplified by removing
the chained code. Asymmetric crypto represents some challenges related
to the scheduling of the kernel which haven't been solved yet
completely.}
...
\appsection{Userspace Daemon}

\begin{designchoice}{Daemon messaging format}
