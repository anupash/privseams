...
between the localhost and a peer. It consist of both parties'
\acp{HIT}, \ac{SPI} values required for \ac{ESP} traffic, IPv6
addresses, security keys, keying material to derive new keys and some
state information. It is needed when sending or receiving \ac{IPsec}
...
The \acp{HA} is implemented as hash table to support fast
searching. The hash table is indexed by \ac{HIT}. However, the support
for multiple \ac{IPsec} \ac{SA} support cannot be supported
efficiently just by indexing using \acp{HIT}. Therefore, the HIP
module as an auxiliary hash table that maps inbound \ac{SPI} numbers
to peer \acp{HIT}. It is used only when an incoming \ac{ESP} envelope
arrives and the HIP module need to map it to a certain \ac{HIP}
\ac{SA}. At that moment, the module is not aware of the \acp{HIT}
involved, but knows only the \ac{SPI} value from \ac{ESP} packet. The
host looks up the corresponding \ac{HIT} from auxiliary hash table,
...
address in the IPv6 header. The hook is both in the TCP and IPv6 code. \\
...
we first connect to the peer, the \ac{HIP} module will start the
\ac{HIP} base exchange.
