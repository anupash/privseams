...
chapters were complete, checkout any version before patch-53 from the
main archives. This document will be updated during fall/winter 2004.}
\vspace{2in}
...
Protocol (HIP) architecture adds a cryptographically based
...
identifier, and on the link layer, the hardware address of the network
interface is used. In HIP, the transport layer is modified so that the
connections are identified using the source and destination HIs as
well as the source and destination ports.  HIP then provides a binding
between the HIs and the IP addresses, e.g. using DNS \cite{rfc1034}.
...
There are some other message types in the HLP. The UPDATE Packet
provides the peer system with its new SPI, provides possibly a new
Diffie-Hellman key to produce new keying material, and provides any
intermediate system with the mapping of the old SPI to the
new.

\cite{hip-mm} \textbf{cite must be IETF-HIP-MM} provides a way
for supporting mobility and multihoming using UPDATE packets. UPDATE
packets include then REA (contains a list of addresses of the host),
SEQ, and ACK parameters, and possibly DIFFIE\_HELLMAN parameter. If the
packet contains a NES parameter, rekeying is performed. The existence
of these parameters depend on desired mobility scenario described in
\cite{hip-mm}.

% The HIP Readdress Packet (REA) allows a hos t to notify its partners
% of a change of the IP address (e.g. as a result of mobility). HIP
% Address Check (AC) and Address Check Reply (ACR) packets are used
% both as an address probe and as an implicit acknowledgement to a REA
% packet
...
to affect the transport layer at all. We have achieved this goal.
...
use HIP protocol headers, we have added hooks to some
...
\item[hip\_handle\_esp] When an incoming ESP packet arrives, we need to
  ...
  corresponding HITs. To do IPv6 -> HIT mapping we have to consult our
  Host Association (HA) database. Correct HA is found by using the SPI
  value in the ESP packet as the search key.
...
is added. When the kernel has performed the Duplicate Address
Detection procedure (DAD), we call this hook and notify active HIP
peers about the new address. Events caused by link local addresses are
ignored.
\end{description}
...
The HADB maintains a collection of Host Associations (HA). Each HA holds
necessary information to establish and maintain an HIP association between
two peer hosts. 
HAs consist of both parties' HITs, SPI numbers required for ESP traffic,
security keys, keying material to derive new keys and some state information.
The HAs are distinguished by peer's HIT and own SPI number
(ie. the SPI number that appears in ESP encapsulated packets destined to us).
To be able to access the HAs efficiently, they are stored in two different 
hash tables: one using peer's HIT, and one own SPI number as hash table key.

The former is required, when either we are sending some transport data, or
due to HIP control messages (base exchange, readdressing etc). The latter is
used only when an incoming ESP encapsulated arrives. We need to map it to a
certain HIP connection. At that time, we are not aware of the HITs involved,
but only the SPI number.

% OLD, SPIOK not used
The HA consists of 
The HA has internal state. This state varies between \verb|HIP_HASTATE_VALID|,
\verb|HIP_HASTATE_SPIOK| and \verb|HIP_HASTATE_HITOK|, meaning that the HA
is inserted in both hash tables, inserted in the SPI hash table and inserted 
in the HIT hash table, respectively.

Althogh there are two hash tables, there are only one set of HAs. This entity,
that holds HAs in one or two hash tables, is called the HADB.
...
counter using \verb|hip_put_ha()|. When the reference counter reaches
...
To maintain integrity across the operations involving the HADB or the HAs
added to the HADB, we use a couple of techniques.

First, we guarantee that no HA can be deleted accidentally while some other
thread/process uses it. This is accomplished by having reference counters in
each HA. This counter must be incremented using \verb|hip_hold_ha()| macro 
whenever we receive a reference to the HA. Obviously the reverse holds for 
decrementing the counter using \verb|hip_put_ha()|. When the reference 
counter reaches zero, the memory allocated for the HA is automatically deleted.
A reference to a HA can only be acquired through certain HADB functions. These
guarantee that there cannot be a situation where we would be deleting the
HA (due to its reference counter reaching zero), and some other thread wanting
to acquire the HA.

Second, each HA has its own spin lock. This lock must be locked when the
information in HA is accessed, either for reading of writing.
The two macros (\verb|HIP_LOCK_HA| and \verb|HIP_UNLOCK_HA|) provide the
correct means to handle HAs.
These macros use \verb|spin_lock_bh| and \verb|spin_unlock_bh|, respectively, to
acquire and release the locks and at the same time to disable and enable bottom
handlers.

Third, the two hash tables in HADB both have a global spin lock. Certain HADB
functions use these to isolate the critical sections. The basic policy is to
use these locks only when operating (reading or writing) on the hash table 
itself. The macros that do the job are: \verb|HIP_LOCK_HT| and \verb|HIP_UNLOCK_HT|.

% Miika: mihinköhän tämän laittaisi?

Due to the architecture of HIPL, there are 3 different contexts, in which the
HIP code is executed: bottom handler/softirq, kernel thread and user context.
The softirq context is used when receiving packet from the network. It has the
highest priority. A HIP worker thread is operating in the kernel thread context.
Userspace HIP configuration tools, HIP socket API etc. run in user context.
An incoming network packet will preempt every other context. If a process/thread
in a lower priority context has acquired a lock, it will not be able to release
it.
...
The purpose of a working queue in HIP is to defer processing of
HIP-related messages. The main functionality is executed by a
HIP kernel thread. Networking stack defers packets to
the HIP thread for further processing.
Originally there were two reasons for this.
The first one was to simplify accessing the internal data structures,
and the second one to enable HIP module to be more controllable.
For example, HIP thread's priority can be raised or lowered, giving
it more or less CPU time.
...
Working queue is simple linked list. At the moment it is not bounded, but
it definitely should be, if the code is ever used in a real, production
server. The list is protected by a semaphore. The worker thread sleeps in
the semaphore until (usually the networking stack) defers some job to it.
Basically, this is a producer consumer model. Where the networking stack and
user configuration tools produce jobs for the kernel thread to consume.
The job is usually related to the HIP base exchange, or other control traffic.
The normal data transfer does not defer execution to later, so it does not
use the working queue.
...
Due to the architecture of HIPL, there are theree different contexts, 
in which the
HIP code is executed: bottom handler/softirq, kernel thread and user context.
The softirq context is used when receiving packet from the network. It has the
highest priority. A HIP worker thread is operating in the kernel thread context.
Userspace HIP configuration tools, HIP socket API etc. run in the user context.
...
For the rendezvous server functionality, we had to add a new hash table,
where we store the RendezVous Associations (RVA). The hash table
uses peer's HIT as a key, just like one of the HADB hash tables.

We define few terms to lessen the readibility and comprehensivity of the
rest of the subchapter.
Responder means the host that
asks the rendezvous server for the rendezvous service. Server means the
...
First, the Responder needs to inform the Server, and ask for rendezvous
service. This is accomplished by adding a special control bit (RVS\_CAPABLE)
in I1 packet, as specified in \cite{RVS}.
To the I2 packet, we add an optional RVA\_REQUEST parameter, \textbf{if} and
only if, the Server also used the RVS\_CAPABLE bit in R1 packet.
Otherwise, the Server signals that is not capable of providing the rendezvous
service we asked for. We then end the base exchange.

Having added the RVA\_REQUEST parameter to the I2, we proceed with the rest
steps of building the I2 packet, and send it. Then rest goes as in the normal
base exchange procedure.

On the Server's side, the process is slightly different. Always when a Server
receives I1 packet, it has to check if it is destined for itself, or if it
belongs to a HIP node, that it is serving.
...
multihoming features defined in draft \cite{hip-mm}. Scenarios
...
from some of its network interface(s). Another type of events are
network device events. The HIP module is notified when a network
device goes down or comes up. Actually, the event when network comes
up is ignored, because at that time the interface usually has no
usable addresses, so the modle waits for IPv6 address addition events.
...
When a peer host receives the initial UPDATE packet, it must validate all
addresses listed in the REA parameter. This is achieved by sending a
reply UPDATE packet to each of the addresses listed in the initial UPDATE
packet. This reply UPDATE packet contains ECHO\_REQUEST and SEQ parameters.
