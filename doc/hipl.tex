...
First the HIP module selectes the addresses to be included in a REA
...
specified in the \ac{HIP} drafts~\cite{hiparch,hip,hip-ietf-mm-00}. The most
...
not to affect the transport layers at all or with minimal changes if
it is absolutely necessary. We have achieved this goal quite well,
only some few line long changes needed to be done to other layer code.
...
we have the following hooks in the underlying IPv6 code:
...
\item[hip\_handle\_esp] When an incoming \ac{ESP} packet arrives, we need to
  ...
  corresponding \acp{HIT}. To do IPv6 to \ac{HIT} mapping we have to consult our
...
\item[hip\_handle\_output] An outgoing \ac{ESP} packet has \acp{HIT} in the
  IPv6 header's source and destination address fields. In this
  function we do the \ac{HIT} to IPv6 mapping.  Again, we need to
  consult our \ac{HA} database. This function is called just before
  IPv6 passes the packet to the lower layer functions.
%  \verb|dst\_output|.
% dst_output / maybe_reroute, depends..change
% --> right before the packet is passed to lower layer.

\item[hip\_get\_addr]
This function selects an address from the set of the usable IPv6 addresses of
the peer. The selected address is used as the destination address of
the outgoing packet.

\item[hip\_get\_saddr]
Selects the source HIT when an outbound packet is to be sent
containing only the destination HIT.

\item[hip\_unknown\_spi] This hook is related to an ongoing discussion about
  what to do if an \ac{ESP} packet with an unknown \ac{SPI} value
  arrives at the host. It has been proposed that a NOTIFY packet is
  sent. Current implementation does not do anything.
% If enabled, we assume that the peer has rebooted and send R1.
...
is added. When the kernel has performed the \ac{DAD} successfully, we
call this hook and notify active \ac{HIP} peers about the new
address. Events caused by link local addresses are ignored.
\end{description}
...
if it has a new IPv6 address or an IPv6 address is deleted. As
described in \ref{sec:loc_hip_kernel},
function \verb|hip_handle_ipv6_dad_completed| handles
\ac{DAD} events and function \verb|hip_handle_inet6_addr_del| handles the
events caused by single address deletion.
...
When the actual plans to implement the UPDATE packet and support for
\cite{hip-ietf-mm-00} type of mobility and multihoming were realized,
we assumed that the amount of work needed would be a couple of
months. But in reality, the total amount of work was much more, around
six months.

A part of this considerable delay can be explained with other tasks,
such as project documentation deadlines, preparing and participating
in IETF meetings, and other implementation tasks not related to the
UPDATE packet or mobility.

A major problem in the previous HIPL implementation was the design of
the \ac{HADB}. It was designed in such a way that there could be only
one \ac{SPI} at the time, and the IPv6 address lists were stored in a
unrelated data structure. This meant that whereever \acp{SPI} and IPV6
addresses were handled in the code, that code had to be rewritten
completely. Because \ac{SPI} was stored in an integer variable and the
current implementation uses linked lists, the change was not just a
simple ``search and replace'' procedure. IPv6 addresses of the peer
were stored in a separate list, but now they had to be bound into an
\ac{SPI}, meaning that there is actually list of lists. \ac{SPI} and
IPv6 address list handling functions had to rewritten from scratch
which took quite a lot of time to implement and debug.

Another slowdown was caused by immature state of the mobility draft
at the time we were implementing. The draft were sometimes even
ambiguous or they did not tell what should really be done. Lots of
time was spent on reading between the lines and trying to figure out
what should be the best way to implement required functionality.

However, the unfinished state of the draft was not only a bad
thing. HIPL developers raised some problem issues and proposed
solutions in the Hipsec mailing list. In practise, almost all of the
mobility draft related discussion was between HIPL and the draft
authors. Implementation gave insight what was missing from the
previous draft versions and what could be added into the next
versions.

We can compare the effort required to implement the current and the
previous mobility code even though the basic design of the both
versions is different. Practically, both had to written from
scratch. Some old code could be reused, but the amount was fairly
minimal. Old code was still useful, because it had general and
reusable ideas on how to implement similar data structures and
functionality. If there would not have been any previous work, a lots
of time would have been needed to overcome the same implementation
problems.

Mobility management is still in its first phase. High level issues
such as source address selection and the selection of the preferred
peer address have not been considered so much yet. These are the
similar kind of issues that many other IETF working groups
(e.g. multi6 WG) are trying to solve, too.
