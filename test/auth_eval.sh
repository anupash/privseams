...
cat $OUTPUT_DIR/$DEVICE_TYPE | awk '{if ($2 == "sha1:") printf("%.3f ms\n", $3);}' | tee $STAGING_DIR/$DEVICE_TYPE/sha1
...
cat $STAGING_DIR/$DEVICE_TYPE/sha1 | $STATS_DIR/stats.pl 95 value '(\S+)\s+(ms)' | awk '{if ($1 == "ms") {printf("avg\tstd_dev\tper_sec\n"); printf("%.3f\t%.3f\t%.3f\n", $2, $3, 1000/$2);}}' | tee $RESULTS_DIR/$DEVICE_TYPE/sha1
