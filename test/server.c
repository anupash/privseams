...
*/
static char        hostBfr[1025];     /* For use w/getnameinfo(3).    */
static const char *pgmName;                 /* Program name w/o dir prefix. */
static char        servBfr[32];     /* For use w/getnameinfo(3).    */
static boolean     verbose = false;         /* Verbose mode indication.     */

static int    tSckt[MAXTCPSCKTS];            /* Array of TCP socket descriptors. */
static size_t tScktSize = MAXTCPSCKTS;       /* Size of uSckt (# of elements).   */
static int    uSckt[MAXUDPSCKTS];            /* Array of UDP socket descriptors. */
static size_t uScktSize = MAXUDPSCKTS;       /* Size of uSckt (# of elements).   */
...
static void server_close(int signum)
{
    int i = 0;

    for (i = 0; i < tScktSize; i++) {
        if (tSckt[i]) {
            printf("closing tsocket: %i\n", tSckt[i]);
            close(tSckt[i]);
        }
    }
    for (i = 0; i < uScktSize; i++) {
        if (uSckt[i]) {
            printf("closing usocket: %i\n", uSckt[i]);
            close(uSckt[i]);
        }
    }
    exit(0);
