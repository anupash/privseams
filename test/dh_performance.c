 ...
 * 
 * \note all sw_ paramters are pointers to ouput parameters that are modified by dhp_getopts.
 * 
...
double dhp_stop_benchmark(struct timeval * bench_time){
	...
	return ((now.tv_sec - bench_time->tv_sec) * 1000000 + (now.tv_usec - bench_time->tv_usec)) / 1000000.0;
}
...
int main(int argc, char ** argv){
	...
	if(sw_file_output){
		...
		hip_perf_set_name(perf_set, PS_HASH,"PS_HC_HASHLOOPS_100_PER_ENTRY.csv");
	...
	if(!sw_cpuload){
		...
		for( i = 0; i < sw_create_rsa; i++){
			//printf("Create key %d\n", i);
			if(sw_file_output) hip_perf_start_benchmark(perf_set, PS_RSA_CREATE);
		...
		for( i = 0; i < sw_bench_loops; i++){
			...
			if(sw_file_output) hip_perf_start_benchmark(perf_set, PS_RSA_SIGN);
			err = RSA_sign(NID_sha1, rsa_data, SHA_DIGEST_LENGTH, rsa_sig_pool[i], &sig_len,
			//err = RSA_sign(NID_md5_sha1, rsa_data, 36, rsa_sig_pool[i], &sig_len,
// NID_md5_sha1, buf,36
...
//		       &sig_len, rsa);
		...
		dhp_start_benchmark(&bench_time);
		for( i = 0; i < sw_bench_loops; i++){
			if(sw_file_output) hip_perf_start_benchmark(perf_set, PS_RSA_VERIFY);
			err = RSA_verify(NID_sha1, rsa_data, SHA_DIGEST_LENGTH, rsa_sig_pool[i],RSA_size(rsa_key_pool[i%sw_create_rsa]) , rsa_key_pool[i%sw_create_rsa]);
			//printf("verify %d rsa\n", RSA_size(rsa_key_pool[i%sw_create_rsa]));
			//err = RSA_verify(NID_md5_sha1, rsa_data, 36, rsa_sig_pool[i],RSA_size(rsa_key_pool[i%sw_create_rsa]) , rsa_key_pool[i%sw_create_rsa]);
		...
		for( i = 0; i < sw_create_dsa; i++){
			if(sw_file_output) hip_perf_start_benchmark(perf_set, PS_DSA_CREATE);
	...
	for( i = 0; i < sw_create_dh; i++){
		...
		if(sw_file_output) hip_perf_start_benchmark(perf_set, PS_DH_CREATE);
	...
	for( i = 0; i < sw_bench_loops; i++){
		...
		bn2bin_safe(dh_key_pool[(i+k)%sw_create_dh]->pub_key, pub_key, dh_size);
		if(sw_file_output) hip_perf_start_benchmark(perf_set, PS_DH_SHARE);
		...
		if(sw_print_keys)HIP_HEXDUMP("Shared key:", shared_key, sw_shared_key_len);
		if(sw_cpuload) i = 0;
	} 
	printf("\n");
	bench_secs = dhp_stop_benchmark(&bench_time);
	...
	for( i = 0; i < sw_bench_loops; i++){
		if(sw_file_output) hip_perf_start_benchmark(perf_set, PS_HC_CREATE);
