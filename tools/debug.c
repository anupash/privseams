 ...
 *
 *   set_logtype(LOGTYPE_STDERR); // set logging output to stderr
 *   set_logfmt(LOGFMT_SHORT);    // set short logging format
 *
...
int logtype = LOGTYPE_STDERR;
#else
int logtype = LOGTYPE_SYSLOG;
#endif /* CONFIG_HIP_DEBUG */

#ifdef HIP_LOGFMT_LONG
int logfmt  = LOGFMT_LONG;
#else
int logfmt  = LOGFMT_SHORT;
#endif /* HIP_LONGFMT */

/**
 * set_logtype - set logging to to stderr or syslog
 * @new_logtype: the type of logging output, either LOGTYPE_STDERR or
 *               LOGTYPE_SYSLOG 
 *
 */
void set_logtype(int new_logtype) {
  logtype = new_logtype;
}

/**
 * set_logfmt - set the formatting of log output (short or long)
 * @new_logfmt: the format of the log output, either LOGFMT_SHORT or
 *              LOGFMT_LONG
 *
 */
void set_logfmt(int new_logfmt) {
  logfmt = new_logfmt;
}

/**
 * handle_log_error - handle errors generated by log handling
...
/**
 * vlog - "multiplexer" for correctly outputting all debug messages
 ...
 *
 * This function is to be used only from the debug(), info(), etc debugging
 * functions.
 */
void vlog(int debug_level, char *file, int line, char *function,
  ...
  case LOGTYPE_SYSLOG:
    ...
    break;
  default:
    printed = fprintf(stderr, "vlog(): undefined logtype: %d", logtype);
    exit(1);
  }
 ...
 err:
  handle_log_error(logtype);

}

/**
 * info - output informative (medium level) debugging messages
 * @file:        the file from where the debug call was made        
 * @line:        the line of the debug call in the source file
 * @function:    the name of function where the debug call is located
 * @fmt:         the output format of the debug message as in printf(3)
...
void info(char *file, int line, char *function, char *fmt, ...) {
  va_list args;
  va_start(args, fmt);
  vlog(DEBUG_LEVEL_INFO, file, line, function, fmt, args);
  va_end(args);
}

/**
 * debug - output development (low level) debugging messages
 * @file:        the file from where the debug call was made        
 * @line:        the line of the debug call in the source file
 * @function:    the name of function where the debug call is located
 * @fmt:         the output format of the debug message as in printf(3)
 ...
 */
void debug(char *file, int line, char *function, char *fmt, ...) {
  va_list args;
  va_start(args, fmt);
  vlog(DEBUG_LEVEL_DEBUG, file, line, function, fmt, args);
  va_end(args);
}

/**
 * die - output a fatal error and exit
 * @file:        the file from where the debug call was made        
 * @line:        the line of the debug call in the source file
 * @function:    the name of function where the debug call is located
 * @fmt:         the output format of the debug message as in printf(3)
...
void die(char *file, int line, char *function, char *fmt, ...) {
  va_list args;
  va_start(args, fmt);
  vlog(DEBUG_LEVEL_DIE, file, line, function, fmt, args);
  va_end(args);
  exit(1);
}

/**
 * error - output an error message (high level)
 * @file:        the file from where the debug call was made        
 * @line:        the line of the debug call in the source file
 * @function:    the name of function where the debug call is located
 * @fmt:         the output format of the debug message as in printf(3)
...
void error(char *file, int line, char *function, char *fmt, ...) {
  va_list args;
  va_start(args, fmt);
  vlog(DEBUG_LEVEL_ERROR, file, line, function, fmt, args);
  va_end(args);
}

/**
 * perror_wrapper - a wrapper for perror(3) style calls
 * @file:        the file from where the debug call was made        
 * @line:        the line of the debug call in the source file
 * @function:    the name of function where the debug call is located
...
/**
 * hexdump - print hexdump starting from address @str of length @len
 * @file:        the file from where the debug call was made        
 * @line:        the line of the debug call in the source file
 * @function:    the name of function where the debug call is located
 ...
 */
void hexdump(char *file, int line, char *function, char *prefix,
	     void *str, int len) {
