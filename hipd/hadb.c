...
void hip_init_hadb(void)
{
	...
	hadb_spi_list = hip_ht_init(hip_hash_spi, hip_hadb_match_spi);
...
void hip_hadb_delete_inbound_spi(hip_ha_t *entry, uint32_t spi)
{
	...
	list_for_each_entry_safe(item, tmp, &entry->spis_in, list){ 
	  	if (!spi || item->spi == spi) {
			...
			hip_delete_sa(item->spi, &entry->local_address, &entry->hit_our,
			...
			//    &item->addresses->address + i, AF_INET6);
 			if (item->spi != item->new_spi)
 				hip_delete_sa(item->new_spi, &entry->hit_our, &entry->local_address,
					      AF_INET6, entry->peer_udp_port, 0);
 			if (item->addresses) {
...
void hip_hadb_delete_outbound_spi(hip_ha_t *entry, uint32_t spi)
{
	...
	HIP_DEBUG("entry=0x%p SPI=0x%x\n", entry, spi);
		...
		if (!spi || item->spi == spi) {
			...
			hip_delete_sa(item->spi, &entry->preferred_address, &entry->preferred_address,
				      AF_INET6, 0, entry->peer_udp_port);
			hip_delete_sa(item->new_spi, &entry->preferred_address,&entry->preferred_address,
				      AF_INET6, 0, entry->peer_udp_port);
