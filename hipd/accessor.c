...
int hip_get_peer_hit(struct hip_common *msg, const struct sockaddr_un *src)
{
	...
	ptr = (struct in6_addr *) hip_get_param_contents(msg,
							 HIP_PARAM_IPV6_ADDR);
	...
	if(!oppdb_exist){
		...
		HIP_IFEL(hip_oppdb_add_entry(&phit, &hit_our, xx, xx, src), -1,
	...
	if(entry) {  /* two consecutive base exchanges */
		...
		/* No need to force I1, retransmissions should handle it */
		goto out_err;
	} else {
		HIP_IFEL(hip_oppdb_add_entry(&phit, &hit_our, &peer_ip, NULL,
					     src), -1,
			 "Add db failed\n");
	}
...
int hip_get_pseudo_hit(struct hip_common *msg)
{
	...
	HIP_IFEL(hip_opportunistic_ipv6_to_hit(&ip, &hit,
					       HIP_HIT_TYPE_HASH100),
