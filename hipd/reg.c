			  ...
			  hip_common_t *target_msg)
{
	...
	else
	{
		...
		for(; i < type_count; i++) {
			...
			case HIP_SERVICE_RELAY:
				...
				/* Check if a relay record already exists. If it
				   exists, we must not replace an RVS with
				   FULLRELAY. The client must cancel the RVS
				   first. */
				if(fetch_record != NULL &&
				   fetch_record->type == HIP_RVSRELAY)
				{
					request_got_rejected = 1;
					HIP_INFO("Client is trying to "\
						 "register to relay service "\
						 "without first canceling "\
						 "existing RVS service. "\
						 "Registration rejected.\n");
				}
				...
				} /* We have already inserted one service. */
				else {    
					request_got_rejected = 1;
					HIP_INFO("Client is trying to "\
						 "register multiple times to "\
						 "relay service using one "\
						 "request. Duplicate "\
						 "registration attempt "\
						 "rejected.\n");
				}
	...
	if(accepted_count > 0)
	{
		...
		/* hip_build_param_reg_request(target_msg, val_lifetime,
		   accepted_requests, accepted_count, 0); */
	}
