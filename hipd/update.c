				      ...
				      uint32_t spi_in)
{
	...
	if (IN6_IS_ADDR_V4MAPPED(&entry->local_address)
		...
		memset(&local_addr, 0, sizeof(in6_addr_t));
		memcpy( (char *)&local_addr, (char *)&entry->local_address, sizeof(in6_addr_t));
	}
		    ...
		    int is_add, struct sockaddr* addr)
{
	...
	== IN6_IS_ADDR_V4MAPPED(&daddr)) {
	     ...
	     memcpy( (char *)&saddr, (char *)&entry->local_address, sizeof(saddr));
     } else {
...
int hip_handle_locator_parameter(hip_ha_t *entry,
	...
	list_for_each_safe(item, tmplist, addresses, ii) {
		...
		if (tmp_af == comp_af) {
			...
			memcpy( (char *)&entry->local_address,
			...
			memcpy( (char *)&entry->preferred_address,
...
int hip_build_locators(struct hip_common *msg)
{
    ...
    list_for_each_safe(item, tmp, hadb_hit, i) {
		    ...
		    memcpy( (char *)&locs2[ii].address, &ha_n->local_reflexive_address,
			   sizeof(struct in6_addr));
