...
void hip_send_opp_tcp_i1(hip_ha_t *entry){
	...
	if(ipType == 0){//ipv4
		...
		memcpy( (char *)&ip6_hdr->ip6_src, (char *)&entry->local_address, sizeof(struct in6_addr));
		memcpy( (char *)&ip6_hdr->ip6_dst, (char *)&entry->preferred_address, sizeof(struct in6_addr));
	}
