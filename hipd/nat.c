 ...
 * TODO doxygen header
 * */
static void hip_on_rx_data(pj_ice_sess *ice, unsigned comp_id, void *pkt, pj_size_t size, const pj_sockaddr_t *src_addr, unsigned src_addr_len){
...
static int hip_poll_ice_event(hip_ha_t *ha, void *unused) {
	...
	if (!(((pj_ice_sess*)ha->ice_session)->is_complete)) {
		...
		HIP_IFEL(!(msg = hip_msg_alloc()), -1, "msg alloc");
