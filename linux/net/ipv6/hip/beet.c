...
void hip_init_hadb(void)
{
	...
	hip_beetdb_hit.hold =      hip_hip_beetdb_hold_entry;
	hip_beetdb_hit.put =       hip_hip_beetdb_put_entry;
	hip_beetdb_hit.get_key =   hip_hip_beetdb_get_key_hit;
	...
	hip_beetdb_spi_list.compare =   hip_hip_beetdb_match_spi;
	hip_beetdb_spi_list.hold =      hip_hip_beetdb_hold_hs;
	hip_beetdb_spi_list.put =       hip_hip_beetdb_put_hs;
	hip_beetdb_spi_list.get_key =   hip_hip_beetdb_get_key_spi_list;
...
void hip_uninit_hadb()
{
	 ...
	 * hip_hip_beetdb_remove_state() for a while.
	 *
	...
	for(i = 0; i < HIP_BEETDB_SIZE; i++) {
		list_for_each_entry_safe(ha, tmp, &hip_beetdb_byhit[i], next_hit) {
			...
			hip_hold_ha(ha);
			hip_hip_beetdb_remove_state(ha);
			hip_put_ha(ha);
		}
	}
	...
	for(i = 0; i < HIP_BEETDB_SIZE; i++) {
		...
		list_for_each_entry_safe(hs, tmp_hs, &hip_beetdb_byspi_list[i], list) {
			...
			hip_hip_beetdb_hold_hs(hs);
			//hip_hip_beetdb_delete_hs(hs);
			hip_hip_beetdb_remove_hs2(hs);
			hip_hip_beetdb_put_hs(hs);
			//} else
