 ...
 * are in sync.
 * 
 * this should not alter the external usage of previously existing
 * functions */
...
int hip_hchain_store_init(int* lengths, int lengths_count){
	...
	memcpy(hip_hchain_storage.store_hchain_length + 1, lengths, lengths_count * sizeof(int));
...
int hip_hchain_bexstore_set_item_length(int item_length)
{
	...
	hip_hchain_storage.store_hchain_length[0] = item_length;
...
int hip_hchain_bexstore_fill(int num_new_items){
	...
	// make sure that store is does not contain more than MAX_STORE_COUNT items
	remaining_items = hip_hchain_storage.store_count[0];
	if (remaining_items + num_new_items > MAX_STORE_COUNT)
		num_new_items = MAX_STORE_COUNT - remaining_items;
	
	item_length = hip_hchain_storage.store_hchain_length[0];
...
int hip_hchain_stores_refill(void)
{
	...
	for (i = 0; i < hip_hchain_storage.num_stores; i++)
	{
		...
		} else
		{
			remaining_items = hip_hchain_storage.store_count[0];
			if (remaining_items < MAX_STORE_COUNT * STORE_THRESHOLD)
				bex_store_update = hip_hchain_bexstore_fill(MAX_STORE_COUNT - remaining_items);
		}
	}
