...
static gboolean update_list_value(GtkTreeModel *,
                                  GtkTreePath *,
                                  GtkTreeIter *,
                                  gpointer);
...
void _info_set(const char *str, int safe)
{
    ...
    if (last >= 0) {
        gtk_statusbar_pop(GTK_STATUSBAR(w), last);
    }
...
/**
 * local_add - Add local HIT to all combo boxes and such.
...
out_err:
    ...
    if (err < 0) {
        HIP_DEBUG("Didn't find item from combo box: %s\n", name);
    }
    return err;
}
    ...
    do {
        ...
        if (strcmp(str, group) == 0) {
            ...
            if (err == TRUE) {
                ...
                if (str[0] == ' ') {
                    gtk_tree_store_remove(GTK_TREE_STORE(w), &iter);
                }
            } else if (err == FALSE && strlen(hit) < 1)   {
                hit = lang_get("hits-group-emptyitem");
            ...
            gtk_tree_view_expand_to_path(GTK_TREE_VIEW(widget(ID_RLISTVIEW)),
                                         path);
            err  = 0;
            break;
        }
...
int group_remote_create(const char *name)
{
    ...
    if (err == GTK_RESPONSE_OK) {
        ...
        if (strcmp(lang_get("group-type-accept"), ps) == 0) {
            accept = HIT_ACCEPT;
        } else {              accept = HIT_DENY;
        }
        ps = gtk_combo_box_get_active_text(GTK_COMBO_BOX(widget(ID_NG_TYPE2)));
        if (strcmp(lang_get("group-type2-lightweight"), ps) == 0) {
            lw = 1;
        } else {              lw = 0;
        }

        strcpy(psn, gtk_entry_get_text(GTK_ENTRY(widget(ID_NG_NAME))));
        if (!check_name_group(psn, NULL)) {
            return group_remote_create(psn);
        }
...
/**
 * check_name_hit - Check HIT name. Meaning strip white spaces and check if it
 *                  contains illegal characters or the name is reserved or if
 *                  it already exists
 *
 * @param *name_orig HIT name to be checked
 * @param *re HIT_remote pointer that will be checked against the group found
...
/**
 * check_name_local - Check local hit name. Meaning strip white spaces and
 *                    check if it contains illegal characters or the name is
 *                    reserved or if it already exists
 *
 * @param *name_orig Local HIT name to be checked
 * @param *le HIT_local pointer that will be checked against the local HIT
...
/**
 * check_apply_group - Check apply for group. Displays a dialog to the user
...
/**
 * check_apply_hit - Check apply for hit. Displays a dialog to the user whether
 *                   to apply changes
 *
...
/**
 * check_apply_hit_move - Check apply hit move. Displays a dialog to the user
 *                        whether to apply changes
 *
 * @param *name Name of the group (not used currently)
...
//              tw_set_remote_rgroup_info(hit->g);
    ...
    g_object_set(widget(ID_TW_APPLY),
                 "label",
                 lang_get("tw-button-apply"),
                 NULL);
...
void edit_group_remote(char *group_name)
{
    ...
    if (group->accept == HIT_ACCEPT) {
        ps = lang_get("group-type-accept");
    ...
    if (group->lightweight == 1) {
        ps = lang_get("group-type2-lightweight");
 ...
 *                      URLCPY(r->port, ps);*/
            ...
            if (g && g != r->g) {
                ...
                NAMECPY(ud.old_name, r->name);
                ud.new_name[0] = '\0';
                gtk_tree_model_foreach(widget(ID_RLISTMODEL),
                                       update_tree_value,
                                       &ud);
                /* Add it to new group in list. */
                hit_remote_add(r->name, g->name);
                if (g2->remotec < 1) {
                    hit_remote_add("", g2->name);
                }
            }
