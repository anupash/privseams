...
static int _hit_remote_add(const char *group, const char *name)
{
    ...
    do {
        ...
        if (strcmp(str, group) == 0) {
            ...
            } else if (err == FALSE && strlen(name) < 1)   {
                name = lang_get("hits-group-emptyitem");
...
//	gtk_window_set_default_icon_name("hipmanager.png");
    ...
    w = gtk_dialog_new_with_buttons(lang_get("title-locals"),
                                    NULL,
                                    GTK_DIALOG_MODAL,
                                    lang_get("lhdlg-button-apply"),
                                    GTK_RESPONSE_YES,
                                    lang_get("lhdlg-button-cancel"),
                                    GTK_RESPONSE_NO,
                                    NULL);
    ...
    w = gtk_dialog_new_with_buttons(lang_get("title-newhit"),
                                    NULL,
                                    GTK_DIALOG_MODAL,
                                    lang_get("nhdlg-button-accept"),
                                    GTK_RESPONSE_YES,
                                    lang_get("nhdlg-button-drop"),
                                    GTK_RESPONSE_NO,
                                    NULL);
    ...
    w = gtk_dialog_new_with_buttons(lang_get("title-runapp"),
                                    NULL,
                                    GTK_DIALOG_MODAL,
                                    NULL);
    ...
    w = gtk_dialog_new_with_buttons(lang_get("title-newgroup"),
                                    NULL,
                                    GTK_DIALOG_MODAL,
                                    NULL);
...
void gui_main(void)
{
    gtk_combo_box_append_text(GTK_COMBO_BOX(widget(ID_TWR_RGROUP)),
                              lang_get("combo-newgroup"));
    gtk_combo_box_append_text(GTK_COMBO_BOX(widget(ID_NH_RGROUP)),
                              lang_get("combo-newgroup"));
