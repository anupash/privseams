 ...
 * @param opendht_port Port that is written to the HTTP header
 * @param opendht_ttl TTL that is used with the rm message
 *
 ...
 * @param opendht_port Port that is written to the HTTP header
 * @param opendht_ttl TTL that is used with the rm message
 * @param[out] *put_packet Buffer to which the packet will be stored
 *
 * @return Returns integer -1 on error, on success 0
 */
 ...
 * @param key Key for the openDHT
 * @param host Host address
 * @param port Port written into the HTTP header
 *
 * @return Returns integer -1 on error, on success 0
 */
 ...
 * @param[out] out_value Where the value will be saved
 *
 ...
 * @param[out] out_key Where the key will be saved
 *
...
int opendht_handle_key(unsigned char *key, char *out_key)
{
    ...
    if (inet_pton(AF_INET6, (char *) key, &addrkey.s6_addr) == 0) {
        ...
        while (k < 13) { /*We get the MSB hex byte from tempchar1 and LSB temchar2 */
 ...
 * @param[out] opaque_answer Pointer to memory area where the corresponding value will be saved
 ...
 * @param[out] *hdrr opaque pointer passed to point to the hdrr result
 *
 ...
 *
 * @param *packet Response returned from the lookup service
 * @param[out] *hit Opaque pointer passed to point to the HIT
 *
 * @return status of the operation 0 on success, -1 on failure
 */
 ...
 *
 * @param *packet Response returned from the lookup service
 * @param[out] *hit Opaque pointer passed to point to the ip
 *
 * @return status of the operation 0 on success, -1 on failure
 */
...
/**
 * verify_hddr_lib - It sends the dht response to hipdaemon first appending one more
 *                   user param for holding a structure hdrr_info hdrr_info is used by
 *                   daemon to mark signature and host id verification results to flags
...
int verify_hddr_lib(struct hip_common *hipcommonmsg, struct in6_addr *addrkey)
{
    ...
    HIP_IFEL(hip_build_user_hdr(hipcommonmsg, HIP_MSG_VERIFY_DHT_HDRR_RESP, 0), -1,
