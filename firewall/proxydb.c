...
int hip_proxy_send_pkt(struct in6_addr *local_addr, struct in6_addr *peer_addr,	u8 *msg, u16 len)
{	
...
{	
	...
	if (dst_is_ipv4) {
		...
		HIP_HEXDUMP("msg dump: ", msg, (len - sizeof(struct ip6_hdr) + sizeof(struct ip)));
	} else {
		HIP_DEBUG("Using IPv6 raw socket\n");
		hip_raw_sock = hip_proxy_raw_sock_v6;
		sa_size = sizeof(struct sockaddr_in6);
		msg = (u16 *) HIP_MALLOC(len, 0);
...
void hip_proxy_dump_memory(void *data, u16 len)
{
	u16 i;
	printf("Data in [%p to %p]: ", data, data+len);
	for(i=0; i<len; i++)
