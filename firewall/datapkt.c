...
int hip_datapacket_mode_init(void)
{
	...
	if (!is_init) {
		...
		HIP_IFEL(!(hip_data_packet = (unsigned char *)malloc(ESP_PACKET_SIZE)), -1,
...
int hip_fw_userspace_datapacket_output(const hip_fw_context_t *ctx)
{
	...
	HIP_IFEL(hip_data_packet_mode_output(ctx, &preferred_local_addr,
			&preferred_peer_addr, hip_data_packet, &data_packet_len),
			1, "failed to create HIP_DATA_PACKET_MODE packet");
