...
unsigned long hip_sava_mac_entry_hash(const hip_sava_mac_entry_t * entry) {
  ...
  memcpy( (char *)&addrs[0], (char *)entry->ip, sizeof(struct in6_addr));
...
int hip_sava_mac_entry_add(struct in6_addr *ip, char * mac) {
  ...
  memcpy( (char *)entry->ip, (char *)ip,
  	 sizeof(struct in6_addr));
  
  memcpy( (char *)entry->mac, (char *)mac,
	  MAC_LENGTH);
...
int savah_fw_access(fw_access_t type, const char *ip, const char *mac, fw_marks_t tag, int ip_version)
{
  ...
  case FW_ACCESS_DENY:
    if (ip_version == IP_VERSION_4) 
      iptables_do_command("iptables -t mangle -A PREROUTING -s %s -m mac --mac-source %s -j MARK --set-mark %d", ip, mac, tag);
    else 
      iptables_do_command("ip6tables -t mangle -A PREROUTING  -s %s -m mac --mac-source %s -j MARK --set-mark %d", ip, mac, tag);
    break;
  case FW_ACCESS_ALLOW:
    if (ip_version == IP_VERSION_4)
      iptables_do_command("iptables -t mangle -D PREROUTING -s %s -m mac --mac-source %s -j MARK --set-mark %d", ip, mac, tag);
    else 
      iptables_do_command("ip6tables -t mangle -D PREROUTING  -s %s -m mac --mac-source %s -j MARK --set-mark %d", ip, mac, tag);
    break;
  default:
    err = -1;
    break;
...
int iptables_do_command(const char *format, ...) {
  ...
  HIP_IFEL((err = vasprintf(&cmd, format, vlist)), -1, "Failed to vasprintf");
  va_end(vlist);
