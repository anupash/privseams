...
static 
char * 
__savah_inet_ntop(struct in6_addr * addr);
static 
int 
...
static 
int 
__hip_sava_mac_entries_cmp(const hip_sava_mac_entry_t * entry1,
...
static 
int 
__hip_sava_mac_entry_delete(struct in6_addr * ip) {
...
int 
savah_fw_access(fw_access_t type, struct in6_addr *ip, const char *mac, fw_marks_t tag, int ip_version)
{
  int err = 0;
