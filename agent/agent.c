...
static void sig_catch_int(int signum)
{
    ...
    agent_exit();
    if (force_exit < 1) {
        HIP_ERROR("SIGINT (CTRL-C) caught, exiting agent...\n");
    } else if (force_exit < 2) {
        HIP_ERROR("SIGINT (CTRL-C) caught, still once to terminate brutally.\n");
    } else {
...
int main(int argc, char *argv[])
{
    ...
    if (err) {
        HIP_ERROR("Could not read config file.\n");
    }
