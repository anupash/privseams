 ...
 */
static int connhipd_send_hitdata_to_daemon(struct hip_common *msg,
...
/**
 * connhipd_handle_msg - Handle message from agent socket. Messages are,
 *                       ping reply from daemon, nat on/off, hipd quit,
 *                       adding local HIT, filter I1/R1
 *
                               ...
                               struct sockaddr_un *addr)
{
    ...
    if (type == SO_HIP_AGENT_PING_REPLY) {
        ...
        emsg = (struct hip_common *) hip_get_param_contents(msg,
                                                            HIP_PARAM_ENCAPS_MSG);
        ...
        if (l) {
            ...
            if (ret == 1) {
            /* hit already exist in the database and is accepted so no
             * need to send it to daemon */
                ret = 0;
...
/**
 * connhipd_thread - This function creates a thread that keeps the hipd
 *                   connection alive
 *
...
/**
 * connhipd_run_thread - This function starts the thread to send the agent-hipd
 *                       ping keep alives
 *
